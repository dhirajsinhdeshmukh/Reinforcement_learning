...
class ReplayBuffer(object):
    ...
    def __init__(self,buffer_sample):
        ...
        # self.priority = np.zeros(self.buffer_size, dtype=float)
        self.batch_size = 32
    ...
    # @dispatch(int,tuple,float)
    # def add_to_buffer(self,counter,buffer_sample,loss):
    #     index_buffer = counter % self.buffer_size
    #     self.buffer[index_buffer] = buffer_sample
    #     self.priority[index_buffer] = loss

    # def normalize_priority(self):
    #     self.priority = self.priority

    def sample_batch(self, counter):
        # priority_choose = False
        # if(priority_choose):
        #     len_buffer = np.min(counter,self.buffer_size)
        #     self.priority = self.priority / np.sum(self.priority)
        #     index_array = np.random.choice(len_buffer,batch_size,replace=False,p=self.priority)
        # else:
...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        if args.test_dqn:
            ...
            self.net.load_state_dict(T.load("DQN_wednesday_8.bat"))
    ...
    def save_network(self):
        T.save(self.net.state_dict(), "DQN_wednesday_8.bat")    
        print("target_network updated")

    def load_network(self):
        self.net.load_state_dict(T.load("DQN_wednesday_6.bat"))
        print("network sync")

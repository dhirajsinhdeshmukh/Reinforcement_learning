...
def mc_prediction(policy, env, n_episodes, gamma = 1.0):
    ...
    for i in range(n_episodes):
        ...
        for epi in reversed(episodes):
            ...
            g_val = g_val*gamma + epi[2]

            temp[epi[0]]=g_val

        for key,value in temp.items():
            if key not in returns_sum:
                returns_sum[key]=value
                returns_count[key]=1
...
def epsilon_greedy(Q, state, nA, epsilon = 0.1):
    ...
    # policy[best_action] = (epsilon/nA) + 1 - epsilon
...
def mc_control_epsilon_greedy(env, n_episodes, gamma = 1.0, epsilon = 0.1):
    ...
    for i in range(n_episodes):
        ...
        episodes = [] # generate empty episode list
        done = False
        ...
        for key, value in returns_sum.items():
            Q[key[0]][key[1]] = value/returns_count[key]

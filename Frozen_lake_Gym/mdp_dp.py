...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-8):
    ...
    cond=True
    while(cond):
        value_function1=value_function.copy()
        for st in range(0,nS):
            val=0
            for at in range(0,nA):
                for x in P[st][at]:
                    val+=( policy[st][at] ) * ( x[0] * ( x[2] + ( gamma* value_function[x[1]] ) ) )
            value_function1[st]=val
        
        if(np.allclose(value_function,value_function1,atol=tol,rtol=0)):
            cond = False
        value_function=value_function1.copy()
...
def policy_improvement(P, nS, nA, value_from_policy, gamma=0.9):
    ...
    for st in range(0,nS):
        ...
        action_list[st] = np.argmax(temp_list)
    
    
    for st in range(0,nS):
        new_policy[st][action_list[st]]=1
    ...
    while(cond):
        for st in range(0,nS):
            ...
            for at in range(0,nA):
                ...
                temp_list.append(val)
            V_new[st]=np.max(temp_list)
            action_list[st] = np.argmax(temp_list)
        
        if(np.allclose(V_new,V,atol=tol,rtol=0)):
            for st in range(0,nS):
                policy_new[st][action_list[st]]=1
            cond = False
